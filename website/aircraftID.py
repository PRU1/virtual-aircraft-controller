...
class aircraft:
    ...
    def __init__(self, type, fltno, callsign, status):
        ...
        if status == "ARR": #arrival
            ...
            print(f"{self.fltno}: Toronto tower, {self.callsign} with you at {int(self.altitude)}ft")
            ...
            self.heading = 237
            self.on_ground = True
            print(f"{self.fltno}: Toronto tower, {self.callsign} holding short Runway 23")
        ...
        while True:
                ...
                while True:  # empty the queue if multiple commands arrived
                    ...
                    if cmd == "alt":
                        ...
                        if self.target_alt > self.altitude:
                            print(f"{self.fltno}: Climb and maintain {val}ft, {self.callsign}")
                        else:
                            print(f"{self.fltno}: Descend and maintain {val}ft, {self.callsign}")
                    elif cmd == "heading":
                        self.target_heading = val
                        print(f"{self.fltno}: Turn heading {val}, {self.callsign}")
                    elif cmd == "speed":
                        self.target_speed = val
                        if self.target_speed > self.speed:
                            print(f"{self.fltno}: Increase speed to {val}kt, {self.callsign}")
                        else:
                            print(f"{self.fltno}: Reduce speed to {val}kt, {self.callsign}")
                    elif cmd == "clearL":
                        self.clearL = val
                        print(f"{self.fltno}: Clear to land Runway 23, {self.callsign}")
                    elif cmd == "clearTO":
                        self.clearTO = val
                        print(f"{self.fltno}: Clear for takeoff Runway 23, {self.callsign}")
            except queue.Empty:
                pass
            ...
            if not self.on_ground: #in air only
                ...
                if self.status == "ARR" and 0.424 * self.posx < self.posy < 0.933 * self.posx and self.altitude <= math.sqrt(self.posx ** 2 + self.posy **2) * 1000 and 227 < self.heading < 247:
                    ...
                    if not self.approach_announced:
                        print(f"{self.fltno}: Toronto tower, {self.callsign} on approach Runway 23")
                        self.approach_announced = True
                ...
                if self.status == "DEP" and self.altitude >= 4000:
                    print(f"{self.fltno}: Contact departure 128.8, {self.callsign}")
                ...
                if self.status == "DEP":
                    ...
                    print(f"{self.fltno}: Contact ground 119.1, {self.callsign}")
...
for entry in rawlist:
    ...
    for n in range(int(len(airline_info[-1])/4)):
        fleet.append(airline_info[-1][n * 4:(n + 1) * 4])
    airline_info[-1] = fleet
...
def gen_aircraft_instance(status):
    ...
    callsign = ""
    for word in airline_list[ID][1:-1]:
        callsign += word + " "
    callsign = callsign + str(number)[0] + " " + str(number)[1] + " " + str(number)[2]
    airframe = random.choice(airline_list[ID][-1])
